...
def earth_movers_distance(obs_1: list, obs_2: list, extra_mass: int = 1):
    ...
    return pulp.value(model.objective) + abs(total_mass_1 - total_mass_2) * extra_mass
...
def _clean_histograms(obs_1: list, obs_2: list) -> Tuple[dict, dict]:
    ...
    # Parse [clean_hist_1], adding 0 if the bin is not present, and subtracting the mass that is already in [clean_hist_2]
    clean_hist_1 = {}
    for i in range(max(obs_1) + 1):
        if i in hist_1:
            clean_hist_1[i] = max(hist_1[i] - hist_2.get(i, 0), 0)
    # Parse [clean_hist_2], adding 0 if the bin is not present, and subtracting the mass that is already in [clean_hist_1]
    clean_hist_2 = {}
    for i in range(max(obs_2) + 1):
        if i in hist_2:
            clean_hist_2[i] = max(hist_2[i] - hist_1.get(i, 0), 0)
